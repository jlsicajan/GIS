...
- Create a copy of ``gis/settings/local.py.example``:  
  `cp gis/settings/local.py.example gis/settings/local.py`
...
- Deregister the example app by removing `'gis.apps.ExampleappConfig'` from ``backend/gis/settings/base.py``.
- Adjust ``backend/gis/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
